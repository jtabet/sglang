...
class ServerArgs:
    ...
    def _handle_attention_backend_compatibility(self):
        ...
        if self.attention_backend is None:
            ...
            if self.kv_cache_dtype.startswith("kvarn_"):
                self.attention_backend = "kvarn"
                logger.info(
                    f"KVarN KV cache dtype ({self.kv_cache_dtype}) detected. "
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            ...
            choices=[
                ...
                "kvarn_k4v4_g128", "kvarn_k4v2_g128", "kvarn_k4v4_g64", "kvarn_k4v2_g64",
            ],
            help='Data type for kv cache storage. "auto" will use model data type. "bf16" or "bfloat16" for BF16 KV cache. "fp8_e5m2" and "fp8_e4m3" are supported for CUDA 11.8+. "fp4_e2m1" (only mxfp4) is supported for CUDA 12.8+ and PyTorch 2.8.0+. "kvarn_*" are KVarN compressed KV cache presets (Hadamard+Sinkhorn+RTN, ~4-8x KV compression).',
        )
        parser.add_argument(
            "--enable-fp32-lm-head",
            action="store_true",
