...
class KVCacheConfigurator:
    ...
    def _build_token_to_kv_pool(
        ...
        *,
        ...
        elif get_model().kv_cache_dtype.startswith("kvarn_") and not self.is_draft_worker:
            # KVarN: use NoOp pool. The real K/V storage is in the KVarN
            # backend's tail pool (fp16, small) + int4 compressed cache.
            # The draft worker runs a standard attention backend and needs a
            # regular KV pool — never KVarN's NoOp pool. Its kv_cache_dtype is
            # already resolved to the draft model dtype by
            # configure_kv_cache_dtype (KVarN targets fall back to model dtype).
    ...
    def _profile_available_bytes(self, pre_model_load_memory: int) -> int:
        ...
        if not self.is_draft_worker:
            prefill_chunk = get_schedule().chunked_prefill_size
            if prefill_chunk is None or prefill_chunk <= 0:
                prefill_chunk = get_schedule().max_prefill_tokens
            if get_model().kv_cache_dtype.startswith("kvarn_"):
        ...
        if self.mambaish_config is not None:
            ...
            if (
                get_model().kv_cache_dtype.startswith("kvarn_")
