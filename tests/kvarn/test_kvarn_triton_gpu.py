...
class TestTritonSinkhornGPU:
    ...
    def test_reduces_imbalance_gpu(self):
        ...
        def imbalance(t):
            sc = t.std(dim=-2)
            sr = t.std(dim=-1)
            return (sc.max() / sc.min().clamp_min(1e-8) +
                    sr.max() / sr.min().clamp_min(1e-8)).item()
...
class TestStoreDequantGPU:
    ...
    def test_k_roundtrip_gpu(self):
        ...
        dequant = kvarn_dequant_tile_k(
            result["q_packed_uint8"], result["s_col_K"],
            result["zp_K"], result["s_row_K"], group=G, bits=4,
        )
    ...
    def test_v_roundtrip_gpu(self):
        ...
        dequant = kvarn_dequant_tile_v(
            result["q_packed_uint8"], result["s_col_V"],
            result["s_row_V"], result["zp_V"], head_dim=D, bits=4,
        )
        err = (dequant - tile).abs().mean()
        mag = tile.abs().mean()
        assert err / mag < 0.3
