...
class TestPipelineInvariance:
    ...
    def test_attention_output_preservation(self, bits):
        ...
        k_deq = kvarn_dequant_tile_k(
            k_stored["q_packed_uint8"], k_stored["s_col_K"],
            k_stored["zp_K"], k_stored["s_row_K"], group=G, bits=bits,
        )
        v_stored = kvarn_store_tile_v(v_rot, bits=bits, sinkhorn_iters=8)
        v_deq = kvarn_dequant_tile_v(
            v_stored["q_packed_uint8"], v_stored["s_col_V"],
            v_stored["s_row_V"], v_stored["zp_V"], head_dim=D, bits=bits,
        )
    ...
    def test_store_dequant_roundtrip_improves_with_sinkhorn(self, preset):
        ...
        deq_s = kvarn_dequant_tile_k(
            stored_s["q_packed_uint8"], stored_s["s_col_K"],
            stored_s["zp_K"], stored_s["s_row_K"], group=G, bits=cfg.key_bits,
        )
        ...
        deq_n = kvarn_dequant_tile_k(
            stored_n["q_packed_uint8"], stored_n["s_col_K"],
            stored_n["zp_K"], stored_n["s_row_K"], group=G, bits=cfg.key_bits,
        )
